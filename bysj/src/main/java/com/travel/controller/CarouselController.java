...
public class CarouselController {
    ...
    @ResponseBody
    public Object addCarousel(Carousel carousel, @RequestParam("file")MultipartFile file){
        HashMap<String,String> addResult = new HashMap<String,String>();
        String newFilename = "";
        try {
            newFilename = UploadFileUtils.uploadImgFile(file,"carousel");
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    @ResponseBody
    public Object updateCarousel(Carousel carousel,@RequestParam(value = "file",required = false) MultipartFile file){
        HashMap<String,String> updateResult = new HashMap<String, String>();
        if (file != null){
            try {
                String newFilename = UploadFileUtils.uploadImgFile(file,"carousel");
                carousel.setUrl("../../images/carousel/" + newFilename);
            } catch (IOException e) {
                e.printStackTrace();
                carousel.setUrl("");
