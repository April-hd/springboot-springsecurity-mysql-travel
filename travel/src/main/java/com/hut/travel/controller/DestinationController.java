...
import com.hut.travel.service.CatesService;
import com.hut.travel.service.DestinationService;
import com.hut.travel.service.HotelService;
...
public class DestinationController {
    ...
    @ResponseBody
    public Object addDestination(Destination destination, @RequestParam("file") MultipartFile file, HttpServletRequest request){
        HashMap<String,String> addResult = new HashMap<String, String>();
        String newFilename = "";
        try {
            newFilename = UploadFileUtils.uploadImgFile(file,request,"destination");
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    @ResponseBody
    public Object updateDestination(Destination destination,@RequestParam(value = "file",required = false) MultipartFile file,HttpServletRequest request){
        HashMap<String,String> updateResult = new HashMap<String, String>();
        if (file != null){
            try {
                String newFilename = UploadFileUtils.uploadImgFile(file,request,"destination");
                destination.setUrl("../../images/destination/" + newFilename);
            } catch (IOException e) {
                e.printStackTrace();
            }
