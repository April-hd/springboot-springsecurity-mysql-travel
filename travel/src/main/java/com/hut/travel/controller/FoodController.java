...
import com.hut.travel.pojo.Food;
import com.hut.travel.service.FoodService;
...
public class FoodController {
    ...
    @ResponseBody
    public Object addFood(Food food, @RequestParam("file")MultipartFile file, HttpServletRequest request){
        HashMap<String,String> addResult = new HashMap<String, String>();
        String newFilename = "";
        try {
            newFilename = UploadFileUtils.uploadImgFile(file,request,"food");
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    @ResponseBody
    public Object updateFood(Food food,@RequestParam(value = "file",required = false) MultipartFile file,HttpServletRequest request){
        HashMap<String,String> updateResult = new HashMap<String, String>();
        if (file != null){
            try {
                String newFilename = UploadFileUtils.uploadImgFile(file, request, "food");
                food.setUrl("../../images/food/" + newFilename);
            } catch (IOException e) {
                e.printStackTrace();
            }
