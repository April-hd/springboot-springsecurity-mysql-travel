...
import com.hut.travel.dao.DestinationDao;
...
public class SceneryController {
    ...
    @ResponseBody
    public Object addScenery(Scenery scenery, @RequestParam("file") MultipartFile file, HttpServletRequest request){
        HashMap<String,String> addResult = new HashMap<String, String>();
        String newFilename = "";
        try {
            newFilename = UploadFileUtils.uploadImgFile(file,request,"scenery");
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    @ResponseBody
    public Object updateScenery(Scenery scenery,@RequestParam(value = "file",required = false) MultipartFile file,HttpServletRequest request){
        HashMap<String,String> updateResult = new HashMap<String, String>();
        if (file != null){
            try {
                String newFilename = UploadFileUtils.uploadImgFile(file, request, "scenery");
                scenery.setUrl("../../images/scenery/" + newFilename);
            } catch (IOException e) {
                e.printStackTrace();
            }
