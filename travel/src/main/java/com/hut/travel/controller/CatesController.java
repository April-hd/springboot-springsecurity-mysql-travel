...
public class CatesController {
    ...
    @ResponseBody
    public Object addCate(Cates cate, @RequestParam("file")MultipartFile file, HttpServletRequest request){
        HashMap<String,String> addResult = new HashMap<String, String>();
        String newFilename = "";
        try {
            newFilename = UploadFileUtils.uploadImgFile(file,request,"cate");
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    @ResponseBody
    public Object updateCate(Cates cate,@RequestParam(value = "file",required = false) MultipartFile file,HttpServletRequest request){
        HashMap<String,String> updateResult = new HashMap<String, String>();
        if (file != null){
            try {
                String newFilename = UploadFileUtils.uploadImgFile(file, request, "cate");
                cate.setUrl("../../images/cate/" + newFilename);
            } catch (IOException e) {
                e.printStackTrace();
            }
