...
public class HotelController {
    ...
    @ResponseBody
    public Object addHotel(Hotel hotel, @RequestParam("file")MultipartFile file, HttpServletRequest request){
        HashMap<String,String> addResult = new HashMap<String,String>();
        String newFilename = "";
        try {
            newFilename = UploadFileUtils.uploadImgFile(file,request,"hotel");
        } catch (IOException e) {
            e.printStackTrace();
            hotel.setUrl("");
        }
    ...
    @ResponseBody
    public Object updateHotel(Hotel hotel, @RequestParam(value = "file",required = false) MultipartFile file, HttpServletRequest request){
        HashMap<String,String> updateResult = new HashMap<String, String>();
        if (file != null){
            try {
                String newFilename = UploadFileUtils.uploadImgFile(file,request,"hotel");
                hotel.setUrl("../../images/hotel/" + newFilename);
            } catch (IOException e) {
                e.printStackTrace();
                hotel.setUrl("");
